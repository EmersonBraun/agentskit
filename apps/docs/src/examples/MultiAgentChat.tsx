...
export function MultiAgentChat() {
  ...
  return (
    <div style={{ border: '1px solid var(--ifm-color-emphasis-200)', borderRadius: '12px', overflow: 'hidden', marginBottom: '2rem' }}>
      <div style={{ padding: '0.75rem 1.25rem', borderBottom: '1px solid var(--ifm-color-emphasis-200)', background: 'var(--ifm-color-emphasis-100)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        <span style={{ fontSize: '1rem' }}>🤖</span>
        <strong style={{ fontSize: '0.9rem' }}>Multi-Agent Planner</strong>
        <span style={{ fontSize: '0.8rem', color: 'var(--ifm-color-emphasis-600)' }}>planner → researcher + coder</span>
      </div>
      ...
      <div style={{ padding: '0.75rem', borderTop: '1px solid var(--ifm-color-emphasis-200)', display: 'flex', gap: '0.5rem' }}>
        <input
          ref={inputRef}
          type="text"
          value={input}
          ...
          style={{ flex: 1, padding: '0.5rem 0.75rem', borderRadius: '8px', border: '1px solid var(--ifm-color-emphasis-300)', fontSize: '0.9rem', outline: 'none' }}
        />
        <button onClick={isStreaming ? stopStreaming : send} disabled={!isStreaming && !input.trim()} style={{
          ...
          background: isStreaming ? '#e74c3c' : 'var(--ifm-color-primary)', color: '#fff',
